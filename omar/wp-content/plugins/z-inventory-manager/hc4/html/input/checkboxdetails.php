...
class HC4_Html_Input_CheckboxDetails
{
	...
	{
		$myId = 'hc4-input-checkboxdetails-' . HC4_App_Functions::generateRand(4);

		$value = $this->helper->getValue( $name, $value );

		$checkboxView = $this->inputCheckbox->render( $name, $value, $checked, $label );
		$checkboxView = '<div class="hc4-input-checkboxdetails-toggler">' . $checkboxView . '</div>';
		$detailsView = '<div class="hc4-input-checkboxdetails-detail hc-mx3">' . $details . '</div>';
...
function CheckboxDetailsInput( el ){
	...
	var togglerContainers = el.getElementsByClassName( 'hc4-input-checkboxdetails-toggler' );
	for( ii = 0; ii < togglerContainers.length; ii++ ){
		var togglers = togglerContainers[ii].getElementsByClassName( 'hc4-input-checkbox' );
	}
